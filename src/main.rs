...
fn is_proxy(config: &Config, req: &HttpRequest) -> Option<(String, HttpRequest)> {
    for proxy_path in config.proxy_rules.keys() {
        if let Some(path_match) = req.path.strip_prefix(proxy_path) {
            let url = config.proxy_rules.get(proxy_path).unwrap().clone();
            let mut proxy_req = req.clone();
            proxy_req.path = path_match.to_string();
...
fn serve_proxy(proxy_url: String) -> HttpResponse {
    ...
    match raw_response {
        ...
        Err(_) => return *HttpResponse::new(HttpStatus::NotFound, "not found", None),
    }
}
...
fn fetch(url: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    // Simple HTTP response parsing (very basic)
...
fn main() {
    ...
    server.listen(move |req| {
        ...
        let is_proxy = is_proxy(&config, &req);
        if proxy_only || is_proxy.is_some() {
            let (url, proxy_req) = is_proxy.unwrap();
            return Box::new(serve_proxy(url));
        }
