...
pub struct Logger {
    buffers: Vec<BufWriter<Box<dyn Write + Send>>>,
}

impl Logger {
    pub fn new(writers: Vec<Box<dyn Write + Send>>) -> Logger {
        let buffers = writers.into_iter().map(|wr| BufWriter::new(wr)).collect();
...
#[test]
fn test_basic() {
    let mut logs = Logger::new(vec![Box::new(stdout())]);
